    ...
    <div className="cursor-pointer" onClick={onClick}>
      <Card sx={{padding: 1, boxShadow: 3}}>
        <CardContent>
          ...
          <div className="text-sm text-normal mb-4 ">Đơn hàng: {items?.map((item) => item?.name).join(', ')}</div>
          {deliveryAddress && (
            <div className="text-sm font-normal mb-2">
              <div>Địa chỉ: {deliveryAddress}</div>
            </div>
          )}
          <div className="text-sm font-normal mb-2 text-justify overflow-hidden line-clamp-3">
            <div>Ghi chú: {notes || 'Không có ghi chú'}</div>
          </div>
          <div className="flex flex-row gap-1 items-center">
            <div className="text-sm font-semibold">Tổng tiền:</div>
            <div className="text-sm font-normal">{formatCurrency(totalAmount)}</div>
          </div>
        </CardContent>
